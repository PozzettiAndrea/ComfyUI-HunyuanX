...
class LoadDinoModel:
    ...
    def load(self, model_size, keep_in_memory=True):
        ...
        try:
            ...
            self.conditioner = DinoConditioner(dino_model)
...
class LoadHunyuanDiT:
    ...
    def load(self, model, attention_mode, use_cache):
        ...
        # Check if already loaded
        if use_cache and self.dit_model is not None and self.current_model_path == model_path and self.current_attention_mode == attention_mode:
            print(f"⚡ Using cached DiT model")
            return ((self.dit_model, self.scheduler),)
        ...
        import yaml
        from .hy3dshape.hy3dshape.pipelines import instantiate_from_config
...
class LoadHunyuanVAE:
    ...
    def load(self, model, use_cache):
        ...
        # Check if already loaded
        if use_cache and self.vae is not None and self.current_model_path == model_path:
