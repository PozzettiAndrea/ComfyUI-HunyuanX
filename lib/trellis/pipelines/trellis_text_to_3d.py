...
class TrellisTextTo3DPipeline(Pipeline):
    ...
    def __init__(
        ...
        clip: str = None,
        clip_moudel: str = "openai/clip-vit-large-patch14",
    ):
        if models is None:
            return
        ...
        self.clip=clip
        self.clip_moudel=clip_moudel

    @staticmethod
    def from_pretrained(path: str, skip_cond_model: bool = False) -> "TrellisTextTo3DPipeline":
        """
        Load a pretrained model.

        Args:
            path (str): The path to the model. Can be either local path or a Hugging Face repository.
            skip_cond_model (bool): If True, skip loading the text conditioning model (for external loading)
        """
    ...
    def _init_text_cond_model(self, name: str):
        ...
        with ctx():
            text_encoder_config=CLIPTextConfig.from_pretrained(self.clip_moudel, local_files_only=True)
        model = CLIPTextModel(text_encoder_config)
        tokenizer = AutoTokenizer.from_pretrained(self.clip_moudel)
        
        clip_dict=load_file(self.clip)
        model.load_state_dict(clip_dict,strict=False)
        del clip_dict
        torch.cuda.empty_cache()
        model.eval()
        self.clip_model = model.cuda()
    ...
    def get_cond(self, prompt: List[str]) -> dict:
        ...
        self.clip_model.to('cpu')# unload clip
        return {
            'cond': cond,
            'neg_cond': neg_cond,
        }
