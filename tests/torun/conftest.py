...
# --- Add HunyuanX to sys.path ---
TESTS_DIR = Path(__file__).resolve().parent.parent  # tests/torun -> tests
HUNYUANX_DIR = TESTS_DIR.parent  # tests -> ComfyUI-HunyuanX
TEST_OUTPUTS_DIR = TESTS_DIR / "test_outputs"

if str(HUNYUANX_DIR) not in sys.path:
    sys.path.insert(0, str(HUNYUANX_DIR))
print(f"✅ Added HunyuanX to sys.path: {HUNYUANX_DIR}")
print(f"📂 Test outputs will be saved to: {TEST_OUTPUTS_DIR}")
...
def setup_test_images():
    ...
    # Get paths
    hunyuanx_root = HUNYUANX_DIR
    examples_dir = hunyuanx_root / "examples"

    # ComfyUI input directory (3 levels up from HunyuanX: custom_nodes -> ComfyUI, then into input/)
    comfyui_root = hunyuanx_root.parent.parent
...
def temp_mesh_file(tmp_path):
    ...
    bunny_path = HUNYUANX_DIR / "examples" / "Stanford_Bunny.stl"
