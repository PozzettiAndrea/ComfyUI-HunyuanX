...
class WorkflowConverter:
    ...
    def _auto_fix_workflow(self, api_workflow: Dict[str, Any]) -> None:
        """
        Automatically fix common workflow validation issues.

        Fixes applied:
        - Clamp num_samples to valid range (1-4) in Trellis_SparseStructure_Sampler nodes

        Args:
            api_workflow: API-format workflow dict (modified in-place)
        """
        for node_id, node in api_workflow.items():
            class_type = node.get("class_type", "")

            # Fix num_samples in Trellis sampler nodes
            if class_type == "Trellis_SparseStructure_Sampler":
                inputs = node.get("inputs", {})

                # Find num_samples parameter (could be "num_samples" or "param_4")
                num_samples_key = None
                num_samples_value = None

                if "num_samples" in inputs:
                    num_samples_key = "num_samples"
                    num_samples_value = inputs["num_samples"]
                elif "param_4" in inputs:
                    # Fallback: if API call failed, widgets_values[4] becomes param_4
                    num_samples_key = "param_4"
                    num_samples_value = inputs["param_4"]

                if num_samples_key and num_samples_value is not None:
                    # Clamp to valid range (1-4), using 3 as default if > 4
                    if num_samples_value > 4:
                        print(f"⚠️  Auto-fix: Node {node_id} ({class_type}): num_samples {num_samples_value} > 4, setting to 3")
                        inputs[num_samples_key] = 3
                    elif num_samples_value < 1:
                        print(f"⚠️  Auto-fix: Node {node_id} ({class_type}): num_samples {num_samples_value} < 1, setting to 1")
                        inputs[num_samples_key] = 1
    ...
    def convert_ui_to_api(self, workflow: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.is_api_format(workflow):
            # Apply auto-fixes even if already in API format
            self._auto_fix_workflow(workflow)
        ...
        # Apply auto-fixes to the converted workflow
        self._auto_fix_workflow(api_workflow)
